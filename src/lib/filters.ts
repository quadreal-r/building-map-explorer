...
/** Unique sorted values for filter dropdown population. */
export function collectFilterOptions(buildings: Building[]): {
  parks: string[]
  clusters: string[]
  managers: string[]
} {
  const parks = [...new Set(buildings.map((b) => b.park))].sort()
  const clusters = [
    ...new Set(buildings.map((b) => b.cluster).filter(Boolean)),
  ].sort()
  const managers = [
    ...new Set(buildings.map((b) => b.manager).filter(Boolean)),
  ].sort()
