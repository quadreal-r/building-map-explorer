...
if (!n || n < 1 || n > 11) {
  console.error('Usage: node export-batch-json.mjs <1-11>')
  process.exit(1)
}
const batchFile = path.join(root, 'supabase', '.seed-chunks', '_batches', `batch-${String(n).padStart(2, '0')}.sql`)
const sql = fs.readFileSync(batchFile, 'utf8')
const out = path.join(root, '.tmp-batch-export.json')
